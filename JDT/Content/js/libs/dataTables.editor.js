...
Editor.prototype.edit = function ( row, title, buttons, show )
{
	...
	this._callbackFire( 'onInitEdit', [row, data] );
...
Editor.prototype.order = function ( set /*, ... */ )
{
	...
	// @todo This and from _display should be a function
	if ( this.s.displayed ) {
		var that = this;
...
Editor.prototype.remove = function ( rows, title, buttons, show )
{
	...
	this._callbackFire( 'onInitRemove', [rows, data] );
...
Editor.prototype._submit = function ( successCallback, errorCallback, formatdata, hide )
{
	...
	eventRet = this._callbackFire( 'onPreSubmit', [data, this.s.action] );
	...
	this.s.ajax(
		...
		data,
		function (json) {
			that._callbackFire( 'onPostSubmit', [json, data, that.s.action] );
	...
	"_addOptions": function ( conf, opts ) {
		...
		if ( opts ) {
			for ( var i=0, iLen=opts.length ; i<iLen ; i++ ) {
				...
				$('input:last', jqInput).attr('value', pair.val)[0]._editor_val = pair.val;
			}
		}
	},
